...
export function usePractice(data: PageData) {
	...
	// Utilities
	let audio = $state<AudioController>(createAudioController(settings.audioEnabled));
	let wakeLockManager = $state<WakeLockManager>(createWakeLockManager(settings.keepAwake));
	...
	let progress = $derived(totalSets > 0 ? (completedSetsCount / totalSets) * 100 : 0);
	...
	function checkAndStartActiveSetTimer() {
		if (!hasStarted || !settings.autoPlay || isReadOnly || isAutoCompletingSet || isAutoAdvancing || isRestTimerRunning) {
			stopActiveSetTimer();
			return;
		}
	...
	function handleAutoPlay(rm: MovementData, setNumber: number, side: Side, movementIndex: number) {
	...
	// Adjust sets
	async function adjustSets(routineMovementId: string, direction: 'up' | 'down') {
		...
		if (response.ok) {
			const rm = data.allRoutineMovements.find((m) => m.id === routineMovementId);
			if (rm) {
	...
	// Reorder movement
	async function reorderMovement(routineMovementId: string, direction: 'up' | 'down') {
	...
	return {
		...
		get routineId() { return routineId; },
		get hasStarted() { return hasStarted; },
		get isPaused() { return isPaused; },
		get isReadOnly() { return isReadOnly; },
		get settings() { return settings; },
		get completedSets() { return completedSets; },
		get skippedSets() { return skippedSets; },
		get completedValues() { return completedValues; },
		get setOverrides() { return setOverrides; },
		get movementNotes() { return movementNotes; },
		get activeMovementIndex() { return activeMovementIndex; },
		get duration() { return duration; },
		get showRestTimer() { return showRestTimer; },
		get restType() { return restType; },
		get nextExerciseName() { return nextExerciseName; },
		get restingMovementIndex() { return restingMovementIndex; },
		get activeRestSetNumber() { return activeRestSetNumber; },
		get activeRestSide() { return activeRestSide; },
		get activeRestType() { return restType; },
		get restTimerValue() { return restTimerValue; },
		get restDurationValue() { return restDurationValue; },
		get isRestTimerRunning() { return isRestTimerRunning; },
		get activeSetTimerValue() { return activeSetTimerValue; },
		get activeSetTimerDuration() { return activeSetTimerDuration; },
		get activeSetTimerPaused() { return activeSetTimerPaused; },
		get isCompletingSet() { return isCompletingSet; },
		get isCompletingWorkout() { return isCompletingWorkout; },
		get isSavingSettings() { return isSavingSettings; },
		get settingsError() { return settingsError; },
		get isAdjustingSets() { return isAdjustingSets; },
		get isReordering() { return isReordering; },
		get isRemoving() { return isRemoving; },
		get isAddingMovement() { return isAddingMovement; },
		get notesSavingStates() { return notesSavingStates; },
		get wakeLockError() { return wakeLockManager.error; },

		// Derived values
		get totalSets() { return totalSets; },
		get completedSetsCount() { return completedSetsCount; },
		get allSetsComplete() { return allSetsComplete; },
		get activeMovement() { return activeMovement; },
		get isInRestPeriod() { return isInRestPeriod; },
