...
import type { MovementInput, MovementUpdateInput } from '$lib/validation/schemas/movement';
...
export async function createMovement(
	...
	await db.insert(movements).values({
		...
		isCustom: true,
		userId,
		weightUnit: (data.type === 'weighted' || data.type === 'resistance_band') && data.defaultUnit
			? data.defaultUnit
			: null,
		isBilateral: data.isBilateral,
		switchSidesDuration: data.switchSidesDuration,
		timePerRep: data.type !== 'timed' && data.timePerRep ? data.timePerRep : null,
...
export async function updateMovement(
	...
	await db.update(movements).set({
		...
		illustrationPath,
		weightUnit: (type === 'weighted' || type === 'resistance_band') && data.defaultUnit
			? data.defaultUnit
			: existing.weightUnit,
		...
		timePerRep: type !== 'timed' && data.timePerRep !== undefined
			? data.timePerRep
			: existing.timePerRep,
