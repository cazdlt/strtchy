...
export const actions = {
  default: async ({ request, locals }: RequestEvent) => {
    ...
    if (!parsed.success) {
      ...
      const missingFields = errorIssues.filter(
        (issue) =>
      ...
      const typeError = errorIssues.find((issue) =>
      ...
      const valueError = errorIssues.find((issue) =>
      ...
      const fileError = errorIssues.find((issue) =>
        issue.path.includes("illustration"),
      );
      if (fileError) {
