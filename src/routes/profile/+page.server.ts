...
export const actions = {
  ...
  updateProfile: async ({ request, locals }: RequestEvent) => {
    ...
    try {
      await db.update(user).set(updateData).where(eq(user.id, locals.user.id));
  ...
  deleteAccount: async ({ request, locals, cookies }: RequestEvent) => {
    ...
    try {
      ...
      await db.delete(routines).where(eq(routines.userId, locals.user.id));

      await db.delete(movements).where(eq(movements.userId, locals.user.id));
  ...
  rotateApiKey: async ({ locals }: RequestEvent) => {
    ...
    try {
      ...
      return {
        success: true,
        message:
          "API key rotated successfully. Copy your new key now - it won't be shown again!",
        apiKey: plaintext,
      };
    } catch (error) {
