...
import {
  updateRoutineMovement,
  deleteRoutineMovement,
} from "$lib/db/helpers/routines";
...
export async function PATCH(event: RequestEvent) {
  try {
    ...
    await updateRoutineMovement(
      event.params.id,
      event.params.routineMovementId,
      parsed.data,
    );
