...
import { routineSchema, routineMovementConfigSchema } from '$lib/validation/schemas/routine';
...
function validateMovementFields(movements: RoutineMovementConfig[]): { valid: boolean; errors: string[] } {
	const errors: string[] = [];
	...
	for (let i = 0; i < movements.length; i++) {
		...
		if (!m.target || typeof m.target !== 'object') {
			...
			if (m.target.value === undefined || m.target.value === null || m.target.value <= 0) {
...
export const actions: Actions = {
	default: async ({ request, locals }) => {
		...
		try {
			transformedMovementsData = transformMovementsData(rawMovementsData as unknown[]);
		} catch (e) {
			console.error('[Routine Create] Failed to transform movements data:', e);
			return fail(400, {
				error: 'Invalid movements data structure. Please check your selections.',
				submittedData
			});
		}
		...
		if (!movementValidation.valid) {
			console.error('[Routine Create] Movement validation failed:', movementValidation.errors);
			return fail(400, {
		...
		if (!parsed.success) {
			...
			console.error('[Routine Create] Schema validation failed:', parsed.error.issues);
			...
			if (errorIssues.find(issue => issue.path.includes('restBetweenMovements'))) {
			...
			const restError = errorIssues.find(issue =>
				issue.path.includes('restBetweenMovements') || issue.path.includes('restBetweenSets')
			);
			if (restError) {
			...
			const movementsError = errorIssues.find(issue =>
				issue.path.includes('movementsData') && issue.message.includes('one movement')
			);
			if (movementsError) {
		...
		try {
			...
			return fail(500, { error: 'Failed to create routine. Please try again.', submittedData });
		}
	}
};
