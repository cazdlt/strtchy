...
import { routineUpdateSchema, routineMovementConfigSchema } from '$lib/validation/schemas/routine';
import { z } from 'zod';
...
export const actions: Actions = {
	default: async ({ request, locals, params }) => {
		...
		if (!movementValidation.valid) {
			console.error('[Routine Edit] Movement validation failed:', movementValidation.errors);
			return fail(400, {
		...
		try {
			...
			return fail(500, { error: 'Failed to update routine. Please try again.', submittedData });
		}
	}
};
